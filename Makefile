...
pack: make_build
	rm -f pp_cmd_str_match-*.tar.gz
	echo Create archive \"pp_cmd_str_match-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_str_match-$(VERSION)-$(BRANCH).tar.gz str_match

clean_pack:
	rm -f pp_cmd_str_match-*.tar.gz


pp_cmd_str_match.tar.gz: build
	cd make_build; tar czf ../pp_cmd_str_match.tar.gz str_match && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./str_match make_build
	cp *.md make_build/str_match/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/str_match
